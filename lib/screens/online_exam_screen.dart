...
class OnlineExamScreen extends StatelessWidget {
  ...
  // قائمة وهمية للامتحانات الأونلاين (يمكن استبدالها ببيانات من API لاحقاً)
  ...
  // دالة لفتح اللينك في المتصفح
  Future<void> _launchURL(String url) async {
    final Uri uri = Uri.parse(url);
    if (await canLaunchUrl(uri)) {
      await launchUrl(uri, mode: LaunchMode.externalApplication);
    } else {
      // يمكن إضافة SnackBar هنا لإظهار رسالة خطأ
  ...
  Widget build(BuildContext context) {
    // فلترة الامتحانات بناءً على examName إذا كنت عايز تعرض امتحانات معينة
